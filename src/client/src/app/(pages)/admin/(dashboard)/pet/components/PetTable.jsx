...
import { adminPetList } from "@/lib/adminAPI/pet";

export default function PetTable({ filter, itemsToDelete, setItemsToDelete, setTotalPages }) {
  ...
  const isAllChecked =
    petList.length > 0 &&
    petList.every((item) => itemsToDelete.includes(item.id));
  ...
  const handleCheckAll = (checked) => {
    if (checked) {
      setItemsToDelete(petList.map((item) => item.id));
    } else {
      setItemsToDelete([]);
    }
  };
      ...
      <table className="min-w-full text-sm text-gray-700">
        <thead className="bg-gray-50 text-gray-600 font-medium border-b">
          <tr>
            ...
            <th className="p-3 text-left">Tên thú cưng</th>
            ...
            petList.map((item) => (
              ...
              >
                ...
                <td className="p-3 flex items-center justify-center gap-2">
                  <AdminEditButton link={`/admin/pet/edit/${item.id}`} />
                  <AdminDeleteButton api={`${process.env.NEXT_PUBLIC_API_URL}/admin/pet/delete/${item.id}`} />
                </td>
              </tr>
            ))}
        </tbody>
